...
**启动任务、监督运行和深入检查，对界面与注意力的要求并不相同。** 把它们全部放进一个聊天窗口，意味着我必须用最重的界面完成最轻的确认。

快捷任务入口与任务监视器，就是对这层关系的重新拆分。

## 快捷任务入口：从工作上下文开始

在桌面上，一项任务通常由明确的工作上下文触发：一个目录、一个仓库，或者几份刚刚选中的文件。
...
Pi Companion 本来就是我自己的 Pi 桌面 GUI，因此也带有明确的个人偏好：Windows、资源管理器入口、常驻桌面的轻量窗口，以及我自己的任务处理方式。它首先服务于我的工作流。
...
真正实现这套体验后，我发现关键在于确保三个界面看到的是同一个任务。
...
Pi Companion 是一套仍在真实任务中持续检验的智能体界面方案。对我而言，它首先是一个可以每天使用的 Pi 桌面 GUI；快捷任务入口和任务监视器，则是其中两个持续验证的设计方向。
