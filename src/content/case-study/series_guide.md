...
category: 'C端产品设计'
featured: true
image: '/series_guide_logo.webp'
demo: 'https://seriesguidedemo-amgov01ad.maozi.io/'
---

---

# Case Study：入坑助手 —— 重新定义复杂影视/游戏系列的观看指南

## 一、项目背景与痛点

### 1.1 问题发现

随着漫威电影宇宙（33部+）、最终幻想（16部正传+数十部衍生）、星球大战等长线IP的不断扩张，影视和游戏系列的作品关系变得错综复杂。在观察用户的实际观影行为时，我发现了一个普遍痛点：**入坑门槛过高**。
...
- 在【怀旧路线】中，玩重制版之前，可能需要先玩原版来理解对比价值
- 在【新玩家路线】中，重制版是一个完美的起点，不需要任何前置
...
1. **软锁机制**：未完成前置的节点显示灰色锁定图标，为需要按部就班的用户提供视觉指引。
2. **强制自由**：用户点击进入作品详情后，可以随时强制标记"已看"，系统不做硬性拦截。
3. **解耦复杂条件**：同一个作品在不同路线下可以有完全不同的前置设置，且不污染基础数据。
...
- **移动端适配**：将查看模式迁移至移动端，保持核心体验一致
- **路线推荐**：根据用户的观看偏好（如"只看主线"、"跳过评分低的"），智能推荐适合的路线
- **社区沉淀**：当用户完成一条路线后，引导其创建自己的路线，形成UGC循环
